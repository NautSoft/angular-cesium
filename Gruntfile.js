...
module.exports = function (grunt) {
	...
	//Common cesiumjs module containing all modules for src and templates
	...
	function findModule(name) {
		...
		var module = {
			name: name,
			moduleName: enquote('cesiumjs.' + name),
		...
		grunt.file.expand('src/' + name + '/*.js')
			...
			.forEach(function (contents) {
				...
				dependencies.split(', ').forEach(function (dep) {
					if (dep.indexOf('cesiumjs.') > -1) {
						var depName = dep.trim().replace('cesiumjs.', '').replace(/['"]/g, '');
	...
	grunt.registerTask('build', 'Create CesiumJS build files', function () {
		var _ = grunt.util._;
